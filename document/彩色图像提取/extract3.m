...
% 读取合并后的RGB图
Img = imread('D:\Matlab\document\彩色图像提取\4.png');
figure;imshow(Img,[]);title('合并后的图像');
%读取隐藏图像
image = imread('D:\Matlab\document\彩色图像提取\13.png');
...
imwrite(uint8(Imgmarkextract), 'D:\Matlab\document\彩色图像提取\5.png'); % 保存图片


img = im2double(imread('D:\Matlab\document\彩色图像提取\5.png'));
...
for i=1:height/m
    for j=1:width/n
        ...
        % 1） 分块
        block(:,:,count) = img((i-1)*m+1:m*i,(j-1)*n+1:j*n,:); % 图像分成块
        count = count+1;
    end
end
imwrite(uint8(Imgmarkextract), 'D:\Matlab\document\彩色图像提取\15.png'); % 保存图片
figure;imshow(block);title('提取出的隐藏图');